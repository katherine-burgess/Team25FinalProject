...
/**
 * This class will log the user's workout statistics. The class will check to see that the user has entered a number, if not a specific
 * error message will be thrown. This class will compare the new workout statistics with the user's goals. If the workout statistics are
 * better than the goals, the user will receive an achievement message. Each workout will be tracked and saved in an ArrayList. 
 * 
...
public class WorkoutComponent {
	...
	// This constructor will check the user input for possible errors, if no errors in input the instance variables will be set. 
	// Otherwise an error will be thrown with a specific message to the user.
	public WorkoutComponent(String dur, String calories, String weight, String intensity, String type) throws InvalidEntryException {
		try {
			...
			if (this.weight < 0 || this.weight > weightMax) {
				throw new InvalidEntryException(String.format("Invalid weight entry. Enter a number between 0 and %d ", weight));
			}
	...
	// This method will compare the user's newest workout statistics to their goals. If a goal is met, 
